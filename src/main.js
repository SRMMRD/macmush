...
async function loadWorlds() {
  try {
    worlds = await invoke('list_worlds');
    renderWorldsList();
  } catch (error) {
    console.error('Failed to load worlds:', error);
  }
...
function renderWorldsList() {
  ...
  if (worlds.length === 0) {
    worldList.innerHTML = '<p class="empty-state">No worlds yet</p>';
    return;
  }

  worldList.innerHTML = worlds.map((world, index) => {
    return `
      <div class="world-item">
        <div class="world-header">
          <span class="world-name">${escapeHtml(world.name)}</span>
          <div class="world-actions">
            <button class="btn btn-icon btn-sm" onclick="connectToWorldFromList('${world.id}')" title="Connect">🔌</button>
            <button class="btn btn-icon btn-sm" onclick="editWorld('${world.id}')" title="Edit">✏️</button>
            <button class="btn btn-icon btn-sm btn-danger" onclick="deleteWorld('${world.id}')" title="Delete">🗑️</button>
          </div>
        </div>
        <div class="world-info">
          <span class="world-host">${escapeHtml(world.host)}:${world.port}</span>
          ${world.use_tls ? '<span class="world-badge">🔒 TLS</span>' : ''}
          ${world.auto_connect ? '<span class="world-badge">⚡ Auto</span>' : ''}
        </div>
        ${world.description ? `<div class="world-description">${escapeHtml(world.description)}</div>` : ''}
      </div>
    `;
  }).join('');
}
...
/**
 * Show world modal for creating/editing
 */
function showWorldModal(worldId = null) {
  editingWorldIndex = worldId;
  const modal = document.getElementById('world-modal');
  const form = document.getElementById('world-form');
  const title = document.querySelector('#world-modal .modal-header h2');
  const submitBtn = document.querySelector('#world-form button[type="submit"]');
  ...
  if (worldId) {
    ...
    submitBtn.textContent = 'Update World';
    document.getElementById('world-name').value = world.name;
    document.getElementById('world-host').value = world.host;
    document.getElementById('world-port').value = world.port;
    document.getElementById('world-description').value = world.description || '';
    document.getElementById('world-timeout').value = world.timeout_secs || 30;
    document.getElementById('world-auto-connect').checked = world.auto_connect || false;
    document.getElementById('world-use-tls').checked = world.use_tls || false;
  } else {
    // Create mode
    title.textContent = 'Create World';
    submitBtn.textContent = 'Create World';
    form.reset();
    document.getElementById('world-timeout').value = 30;
  }
...
function hideWorldModal() {
  ...
  editingWorldIndex = null;
}

/**
 * Handle create/edit world form submission
 */
async function handleCreateWorld(event) {
  event.preventDefault();

  const name = document.getElementById('world-name').value.trim();
  const host = document.getElementById('world-host').value.trim();
  const port = parseInt(document.getElementById('world-port').value);
  const description = document.getElementById('world-description').value.trim();
  const timeout_secs = parseInt(document.getElementById('world-timeout').value);
  const auto_connect = document.getElementById('world-auto-connect').checked;
  const use_tls = document.getElementById('world-use-tls').checked;
  const errorEl = document.getElementById('world-error');

  if (!name) {
    errorEl.textContent = 'World name is required';
    return;
  }
  ...
  try {
    if (editingWorldIndex) {
      // Update existing world
      worldData.id = editingWorldIndex;
      const updatedWorld = await invoke('update_world', { request: worldData });
      appendOutput(`✓ World "${name}" updated`, 'system');
    } else {
      // Create new world
      const newWorld = await invoke('create_world', { request: worldData });
      appendOutput(`✓ World "${name}" created`, 'system');
    }
...
async function deleteWorld(worldId) {
  ...
  try {
    await invoke('delete_world', { id: worldId });
    appendOutput(`✓ World "${world.name}" deleted`, 'system');
    await loadWorlds();
  } catch (error) {
    console.error('Failed to delete world:', error);
    appendOutput(`❌ Failed to delete world: ${error}`, 'error');
  }
}

/**
 * Connect to a world from the worlds list
 */
async function connectToWorldFromList(worldId) {
  const world = worlds.find(w => w.id === worldId);
  if (!world) {
    console.error('World not found:', worldId);
    return;
  ...
  try {
    await connect(world.name, world.host, world.port, world.use_tls);
  } catch (error) {
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  clientInterface = document.getElementById('client-interface');
  ...
  addTriggerBtn = document.getElementById('add-trigger-btn');
  closeTriggerBtn = document.getElementById('close-modal-btn');
  cancelTriggerBtn = document.getElementById('cancel-trigger-btn');
  triggerError = document.getElementById('trigger-error');

  // Test Trigger UI elements
  testTriggerModal = document.getElementById('test-trigger-modal');
  testTriggerForm = document.getElementById('test-trigger-form');
  ...
  // Saved Worlds UI elements
  savedWorldsList = document.getElementById('saved-worlds-list');
  saveWorldBtn = document.getElementById('save-world-btn');

  // Recent Connections UI elements
  recentConnectionsList = document.getElementById('recent-connections-list');
  ...
  closeAliasModalBtn = document.getElementById('close-alias-modal-btn');
  cancelAliasBtn = document.getElementById('cancel-alias-btn');
  aliasError = document.getElementById('alias-error');

  // Test Alias UI elements
  testAliasModal = document.getElementById('test-alias-modal');
  testAliasForm = document.getElementById('test-alias-form');
  closeTestAliasModalBtn = document.getElementById('close-test-alias-modal-btn');
  cancelTestAliasBtn = document.getElementById('cancel-test-alias-btn');
  testAliasResult = document.getElementById('test-alias-result');
  ...
  closeMacroModalBtn = document.getElementById('close-macro-modal-btn');
  cancelMacroBtn = document.getElementById('cancel-macro-btn');
  macroError = document.getElementById('macro-error');
  macroKeyInput = document.getElementById('macro-key');

  // Highlight UI elements
  highlightModal = document.getElementById('highlight-modal');
  ...
  closeHighlightModalBtn = document.getElementById('close-highlight-modal-btn');
  cancelHighlightBtn = document.getElementById('cancel-highlight-btn');
  highlightError = document.getElementById('highlight-error');
  ...
  statusBytesSent = document.getElementById('status-bytes-sent');
  statusBytesReceived = document.getElementById('status-bytes-received');
  statusVariables = document.getElementById('status-variables');
  ...
  closeScriptModalBtn = document.getElementById('close-script-modal-btn');
  cancelScriptBtn = document.getElementById('cancel-script-btn');
  scriptError = document.getElementById('script-error');
  ...
  // Tab UI elements
  worldsTab = document.getElementById('worlds-tab');
  triggersTab = document.getElementById('triggers-tab');
  aliasesTab = document.getElementById('aliases-tab');
  macrosTab = document.getElementById('macros-tab');
  ...
  worldsContent = document.getElementById('worlds-content');
  triggersContent = document.getElementById('triggers-content');
  aliasesContent = document.getElementById('aliases-content');
  macrosContent = document.getElementById('macros-content');
  highlightsContent = document.getElementById('highlights-content');
  const scriptsContent = document.getElementById('scripts-content');
  const timersContent = document.getElementById('timers-content');
  ...
  const closeWorldModalBtn = document.getElementById('close-world-modal-btn');
  const cancelWorldBtn = document.getElementById('cancel-world-btn');

  // Set up event listeners
  connectForm.addEventListener('submit', handleConnect);
  saveWorldBtn.addEventListener('click', handleSaveWorld);
  document.getElementById('import-world-btn').addEventListener('click', handleImportWorld);
  document.getElementById('export-world-btn').addEventListener('click', handleExportWorld);
  clearRecentBtn.addEventListener('click', handleClearRecent);
  ...
  addTriggerBtn.addEventListener('click', showTriggerModal);
  closeTriggerBtn.addEventListener('click', hideTriggerModal);
  cancelTriggerBtn.addEventListener('click', hideTriggerModal);
  triggerForm.addEventListener('submit', handleCreateTrigger);
  ...
  // Test Trigger event listeners
  closeTestModalBtn.addEventListener('click', hideTestTriggerModal);
  cancelTestBtn.addEventListener('click', hideTestTriggerModal);
  testTriggerForm.addEventListener('submit', handleTestTriggerSubmit);

  // Close test modal on background click
  testTriggerModal.addEventListener('click', (e) => {
    if (e.target === testTriggerModal) {
      hideTestTriggerModal();
    }
  });

  // Alias event listeners
  addAliasBtn.addEventListener('click', showAliasModal);
  closeAliasModalBtn.addEventListener('click', hideAliasModal);
  cancelAliasBtn.addEventListener('click', hideAliasModal);
  aliasForm.addEventListener('submit', handleCreateAlias);

  // Close alias modal on background click
  ...
  // Test Alias event listeners
  closeTestAliasModalBtn.addEventListener('click', hideTestAliasModal);
  cancelTestAliasBtn.addEventListener('click', hideTestAliasModal);
  testAliasForm.addEventListener('submit', handleTestAliasSubmit);

  // Close test alias modal on background click
  testAliasModal.addEventListener('click', (e) => {
    if (e.target === testAliasModal) {
      hideTestAliasModal();
    }
  });

  // Macro event listeners
  addMacroBtn.addEventListener('click', () => showMacroModal());
  closeMacroModalBtn.addEventListener('click', hideMacroModal);
  cancelMacroBtn.addEventListener('click', hideMacroModal);
  macroForm.addEventListener('submit', handleCreateMacro);
  macroKeyInput.addEventListener('keydown', handleMacroKeyCapture);

  // Close macro modal on background click
  ...
  addHighlightBtn.addEventListener('click', () => showHighlightModal());
  closeHighlightModalBtn.addEventListener('click', hideHighlightModal);
  cancelHighlightBtn.addEventListener('click', hideHighlightModal);
  highlightForm.addEventListener('submit', handleCreateHighlight);
  syncColorInputs();

  // Close highlight modal on background click
  ...
  closeScriptModalBtn.addEventListener('click', hideScriptModal);
  cancelScriptBtn.addEventListener('click', hideScriptModal);
  scriptForm.addEventListener('submit', handleCreateScript);

  // Close script modal on background click
  ...
  addTimerBtn.addEventListener('click', () => showTimerModal());
  closeTimerModalBtn.addEventListener('click', hideTimerModal);
  cancelTimerBtn.addEventListener('click', hideTimerModal);
  timerForm.addEventListener('submit', handleCreateTimer);

  // Close timer modal on background click
  ...
  addWorldBtn.addEventListener('click', () => showWorldModal());
  closeWorldModalBtn.addEventListener('click', hideWorldModal);
  cancelWorldBtn.addEventListener('click', hideWorldModal);
  worldForm.addEventListener('submit', handleCreateWorld);

  // Close world modal on background click
  ...
  worldsTab.addEventListener('click', () => {
    ...
    triggersTab.classList.remove('active');
    aliasesTab.classList.remove('active');
    ...
    worldsContent.classList.add('active');
    triggersContent.classList.remove('active');
    aliasesContent.classList.remove('active');
    macrosContent.classList.remove('active');
    highlightsContent.classList.remove('active');
    scriptsContent.classList.remove('active');
    timersContent.classList.remove('active');
  });

  triggersTab.addEventListener('click', () => {
    worldsTab.classList.remove('active');
    triggersTab.classList.add('active');
    aliasesTab.classList.remove('active');
    macrosTab.classList.remove('active');
    highlightsTab.classList.remove('active');
    scriptsTab.classList.remove('active');
    timersTab.classList.remove('active');
    worldsContent.classList.remove('active');
    triggersContent.classList.add('active');
    aliasesContent.classList.remove('active');
    macrosContent.classList.remove('active');
    highlightsContent.classList.remove('active');
    scriptsContent.classList.remove('active');
  ...
  aliasesTab.addEventListener('click', () => {
    worldsTab.classList.remove('active');
    aliasesTab.classList.add('active');
    triggersTab.classList.remove('active');
    macrosTab.classList.remove('active');
    highlightsTab.classList.remove('active');
    scriptsTab.classList.remove('active');
    timersTab.classList.remove('active');
    worldsContent.classList.remove('active');
    aliasesContent.classList.add('active');
    triggersContent.classList.remove('active');
    macrosContent.classList.remove('active');
    highlightsContent.classList.remove('active');
    scriptsContent.classList.remove('active');
  ...
  macrosTab.addEventListener('click', () => {
    worldsTab.classList.remove('active');
    ...
    timersTab.classList.remove('active');
    worldsContent.classList.remove('active');
    macrosContent.classList.add('active');
    triggersContent.classList.remove('active');
    aliasesContent.classList.remove('active');
    highlightsContent.classList.remove('active');
    scriptsContent.classList.remove('active');
  ...
  timersTab.addEventListener('click', () => {
    ...
    scriptsTab.classList.remove('active');
    worldsContent.classList.remove('active');
    timersContent.classList.add('active');
    triggersContent.classList.remove('active');
    aliasesContent.classList.remove('active');
    macrosContent.classList.remove('active');
    highlightsContent.classList.remove('active');
  ...
  toggleLoggingBtn = document.getElementById('toggle-logging-btn');
  openLogsBtn = document.getElementById('open-logs-btn');
  searchOutputInput = document.getElementById('search-output');
  ...
  filterMudCheckbox = document.getElementById('filter-mud');
  filterSystemCheckbox = document.getElementById('filter-system');
  filterCommandCheckbox = document.getElementById('filter-command');
  filterErrorCheckbox = document.getElementById('filter-error');

  // Output toolbar event listeners
  clearOutputBtn.addEventListener('click', clearOutput);
  toggleLoggingBtn.addEventListener('click', toggleLogging);
  openLogsBtn.addEventListener('click', openLogsFolder);
  scrollToBottomBtn.addEventListener('click', scrollToBottom);
  outputDisplay.addEventListener('scroll', handleOutputScroll);

  // Search event listeners
  searchOutputInput.addEventListener('input', (e) => {
  ...
  clearSearchBtn.addEventListener('click', () => {
    searchOutputInput.value = '';
    clearSearch();
  });

  // Filter event listeners
  filterMudCheckbox.addEventListener('change', (e) => {
    handleFilterToggle('mud', e.target.checked);
  });
  filterSystemCheckbox.addEventListener('change', (e) => {
    handleFilterToggle('system', e.target.checked);
  });
  filterCommandCheckbox.addEventListener('change', (e) => {
    handleFilterToggle('command', e.target.checked);
  });
  filterErrorCheckbox.addEventListener('change', (e) => {
    handleFilterToggle('error', e.target.checked);
  });

  // Global keyboard event listener for macros and shortcuts
  document.addEventListener('keydown', handleKeyboardShortcut);

  // World tab event listeners
  const newWorldBtn = document.getElementById('new-world-btn');
  if (newWorldBtn) {
    newWorldBtn.addEventListener('click', handleNewWorld);
  }

  // Delegate world tab clicks
  const worldTabList = document.getElementById('world-tab-list');
  if (worldTabList) {
    worldTabList.addEventListener('click', (e) => {
      const tab = e.target.closest('.world-tab');
      if (!tab) return;

      const closeBtn = e.target.closest('.world-tab-close');
      if (closeBtn) {
        // Close button clicked
        e.stopPropagation();
        const worldId = closeBtn.getAttribute('data-world-id');
        if (confirm('Close this world?')) {
          closeWorld(worldId);
        }
      } else {
        // Tab clicked - switch to world
        const worldId = tab.getAttribute('data-world-id');
  ...
  await listen('mud-event', handleMudEvent);

  // Check if already connected (from previous session)
  await checkConnectionStatus();

  // Auto-connect to favorite world if not already connected
  if (!isConnected) {
    await checkAutoConnect();
  }

  console.log('MACMush initialized');
});
